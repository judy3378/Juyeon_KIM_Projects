...
class CorridorNavigator:
    ...
    def scan_callback(self, scan_msg):
        # Get left and right distances from LDS
        left_distances = [x for x in scan_msg.ranges[:50]if not (np.isinf(x) and x>0.0001)]
        ...
        right_distances = [x for x in scan_msg.ranges[-50:]if not (np.isinf(x) and x>0.0001)]
